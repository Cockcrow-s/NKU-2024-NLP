...
class GRU(nn.Module):
    ...
    def _sort_tensor(self, input, lengths):
        '''
        pack_padded_sequence requires the length of seq to be in descending order
        to work.
    ...
    def _unsort_tensor(self, input, invert_order, num_zero):
        ...
        if num_zero == 0:
            ...
            zero = torch.zeros((num_zero, dim1, dim2), device=input.device, dtype=input.dtype)
    ...
    def forward(self, text, text_len):
        ...
        text, _ = self.gru(text)         # batch_size, max_doc_len, hidden_dim * num_directions

        # Unpack the output, and invert the sorting
        text = pad_packed_sequence(text, batch_first=True)[0]  # batch_size, max_doc_len, gru_size
        text = self._unsort_tensor(text, invert_order, num_zero)  # batch_size, max_doc_len, gru_size
