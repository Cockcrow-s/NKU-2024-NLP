...
class BASE(nn.Module):
    ...
    def _compute_cos(self, XS, XQ):
        ...
        # print("query_db_dist.shape", dist.shape)

        return dist

    def compute_cos(XS, XQ):
        '''
            Compute the pairwise cos distance
            @param XS (support x): support_size x ebd_dim
            @param XQ (support x): query_size x ebd_dim
    ...
    def YQ_to_Label(self, YS, YQ):
        ...
        '''
        # print('YS', YS)
        uniques = torch.unique(YS, sorted=False)
        # print('YS', YS, 'uniques', uniques)
        unique1, inv_S = torch.unique(YS, sorted=True, return_inverse=True)
        unique2, inv_Q = torch.unique(YQ, sorted=True, return_inverse=True)
        ...
        Y_new = YQ.clone().detach()

        if self.args.shot == 1:
            for i, way in enumerate(YS):
                # print('i', i, 'way', way)
                Y_new[YQ == way] = i
                # print('Y_new', Y_new)
        return Y_new
