...
def main():
    ...
    parser.add_argument(
        "--max_seq_lens", default=512, type=int, help="Max sequence length."
    )
    ...
    plm, tokenizer, model_config, WrapperClass = load_plm_from_config(
    ...
    mytemplate = ManualTemplate(tokenizer=tokenizer).from_file(
        template_path, choice=args.template_id
    )
    ...
    tot_step = (
        len(train_dataloader) // args.gradient_accumulation_steps * args.max_epochs
    )
    ...
    this_run_unicode = (
        f"{args.dataset}-seed{args.seed}-shot{args.shot}-lr{args.lr}-lr2{args.lr2}-batch_size{args.batch_size}-multi_mask{args.multi_mask}-use_new_ct{args.use_new_ct}-cs_mode{args.cs_mode}-ctl{args.contrastive_logits}"
    ...
    for epoch in range(args.max_epochs):
        ...
        for batch in tqdm(train_dataloader):
            ...
            loss_detailed = [
                loss_detailed[idx] + value
                for idx, value in enumerate(cur_loss_detailed)
            ]
            loss.backward()
            torch.nn.utils.clip_grad_norm_(
                prompt_model.parameters(), args.max_grad_norm
            )
        ...
        scores = prompt_model.evaluate(
            validation_dataloader, processor, desc="Valid", mode=args.eval_mode
        )
        early_stop_count += 1
        if args.eval_full:
            score_str = ""
            ...
            for k in best_record:
                if scores[k] > best_record[k]:
                    ...
                    torch.save(
                        prompt_model.state_dict(), f"ckpts/{this_run_unicode}-{k}.ckpt"
                    )
                    early_stop_count = 0
            ...
            if macro_f1 > best_score_macro:
                ...
                torch.save(
                    prompt_model.state_dict(), f"ckpts/{this_run_unicode}-macro.ckpt"
                )
            ...
            if micro_f1 > best_score_micro:
                ...
                torch.save(
                    prompt_model.state_dict(), f"ckpts/{this_run_unicode}-micro.ckpt"
                )
    ...
    if args.eval_full:
        ...
        for k in best_keys:
            prompt_model.load_state_dict(
                torch.load(f"ckpts/{this_run_unicode}-{k}.ckpt")
            )

            scores = prompt_model.evaluate(
                test_dataloader, processor, desc="test", mode=args.eval_mode, args=args
            )
            tmp_str = ""
        ...
        scores = prompt_model.evaluate(
            test_dataloader, processor, desc="test", mode=args.eval_mode
        )
        macro_f1_1 = scores["macro_f1"]
        micro_f1_1 = scores["micro_f1"]
        acc_1 = scores["acc"]
        print_info(
            "finally best macro {} {} micro {} acc {}".format(
                best_score_macro_epoch, macro_f1_1, micro_f1_1, acc_1
            )
        )
        ...
        acc_2 = scores["acc"]
        print_info(
            "finally best micro {} {} micro {} acc {}".format(
                best_score_micro_epoch, macro_f1_2, micro_f1_2, acc_2
            )
        )
